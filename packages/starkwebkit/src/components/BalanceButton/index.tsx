...
export const Balance: React.FC<BalanceProps> = ({ hideIcon, hideSymbol }) => {
  ...
  const isChainSupported = useChainIsSupported(chain?.chain_id);
  ...
  // let balance = { formatted: '1000000000000000000', symbol: 'ETH' };
  // let queryKey = ['balance'] as const;
  useEffect(() => {
    if (blockNumber ?? 0 % 5 === 0) queryClient.invalidateQueries({ queryKey });
  }, [blockNumber, queryKey]);

  const currentChain = chainConfigs.find((c) => c.chain_id === chain?.chain_id);
  const state = `${
    !isMounted || balance?.formatted === undefined
      ? `balance-loading`
      : `balance-${currentChain?.chain_id}-${balance?.formatted}`
  }`;
      ...
      <AnimatePresence initial={false}>
        ...
        >
          {!address || !isMounted || balance?.formatted === undefined ? (
            <Container>
              {!hideIcon && <Chain chain_id={chain?.chain_id} />}
